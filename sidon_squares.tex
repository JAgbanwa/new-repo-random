...
with all four elements in $\mathcal{A}$, remove one element from $\mathcal{A}$.
The resulting set $\mathcal{A}^*$ satisfies $|\mathcal{A}^*| \ge X - Y$, and
$A^* = \{a^2 : a \in \mathcal{A}^*\}$ is a Sidon subset of $\SN$.  Hence
\[
  \Exp[|\mathcal{A}^*|] \;\ge\;
...
\]
Since $|\mathcal{A}^*|$ is integer-valued, if every outcome had
$|\mathcal{A}^*| < \Exp[|\mathcal{A}^*|]$ then every outcome would have
$|\mathcal{A}^*| \le \lfloor\Exp[|\mathcal{A}^*|]\rfloor$,
forcing $\Exp[|\mathcal{A}^*|] \le \lfloor\Exp[|\mathcal{A}^*|]\rfloor
< \Exp[|\mathcal{A}^*|]$, a contradiction.
Hence there exists a realisation with
\[
