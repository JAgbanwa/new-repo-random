...
\begin{itemize}
  ...
  \item $c = 1$, $\tau(v') = 2$ (so $v'$ is prime, $v = 5v'$):
    ...
    i.e.\ $25 \mid 6\ell + 1 = 25v'$, so $v'$ is an integer (consistent).
    Combined with $\ell \equiv 2 \pmod 3$, CRT over $\mathrm{lcm}(3,25) = 75$
    gives $\ell \equiv 29 \pmod{75}$.  Since $29 \equiv 1 \pmod 7$,
    we compute $v = 5v' \equiv (6\cdot 1 + 1)\cdot 3 = 21 \equiv 0 \pmod 7$
    (using $5^{-1}\equiv 3\pmod 7$), so $7 \mid v = 5v'$, hence $7\mid v'$.
    Since $v'$ is prime, $v' = 7$, giving $v = 35$ and
    $\ell = (175-1)/6 = 29$ (prime). But then
