...
of the six quantities to be composite (or to yield a non-integer, contradicting
primality):
\begin{itemize}
  \item $\ell \equiv 0 \pmod 7$: Combined with $\ell \equiv 4 \pmod 5$, CRT gives
    $\ell \equiv 49 \equiv 14 \pmod{35}$, which is even.  No odd prime satisfies
    this.  Contradiction.
  \item $\ell \equiv 1 \pmod 7$: $v = (6\ell+1)/5 \equiv (6+1) \cdot 3 = 21
    \equiv 0 \pmod 7$ (using $5^{-1} \equiv 3 \pmod 7$).  So $7 \mid v$;
    since $v$ is prime, $v = 7$, giving $6\ell + 1 = 35$, i.e.\ $\ell = 34/6$:
    not an integer.  Contradiction.
  \item $\ell \equiv 2 \pmod 7$: $r = (3\ell+1)/2 \equiv (6+1) \cdot 4 = 28
    \equiv 0 \pmod 7$ (using $2^{-1} \equiv 4 \pmod 7$).  So $7 \mid r$;
    since $r$ is prime, $r = 7$, giving $3\ell + 1 = 14$, i.e.\ $\ell = 13/3$:
    not an integer.  Contradiction.
  \item $\ell \equiv 3 \pmod 7$: $s = 2\ell + 1 \equiv 7 \equiv 0 \pmod 7$.
    Since $s$ is prime, $s = 7$, giving $\ell = 3$.  But $\ell \equiv 4 \pmod 5$
    while $3 \not\equiv 4 \pmod 5$.  Contradiction.
  \item $\ell \equiv 4 \pmod 7$: $q = 3\ell + 2 \equiv 14 \equiv 0 \pmod 7$.
    Since $q$ is prime, $q = 7$, giving $\ell = 5/3$: not an integer.
    Contradiction.
  \item $\ell \equiv 5 \pmod 7$: $p = 6\ell + 5 \equiv 35 \equiv 0 \pmod 7$.
    Since $p$ is prime, $p = 7$, giving $6\ell + 5 = 7$, i.e.\ $\ell = 1/3$:
    not an integer.  Contradiction.
  \item $\ell \equiv 6 \pmod 7$: One verifies $\ell, s, r, q, p, v$ are all
    $\equiv 6 \pmod 7$ (none zero), so this case requires further analysis.
\end{itemize}

Therefore $\ell \equiv 6 \pmod 7$.  Since $6 \equiv -1 \pmod 7$ and we already
have $\ell \equiv -1 \pmod 3$ and $\ell \equiv -1 \pmod 5$, by CRT:
$\ell \equiv -1 \pmod{105}$, i.e.\ $\ell \equiv 104 \pmod{105}$.
